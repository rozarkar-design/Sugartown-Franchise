...
export const AdminPage: React.FC<AdminPageProps> = ({ onDataRefreshed }) => {
  ...
  const [isAuthenticated, setIsAuthenticated] = useState<boolean>(() => {
    return localStorage.getItem('sugartown_admin_auth') === 'true';
  });
  ...
  const [activeTab, setActiveTab] = useState<
    'leads' | 'investments' | 'assumptions' | 'cities' | 'faqs' | 'documents'
  >('leads');
  ...
  const handleLogin = (e: React.FormEvent) => {
    e.preventDefault();
    if (pinCode === 'admin' || pinCode === 'sugartown' || pinCode === 'sugartown2025' || pinCode === '1234') {
      setIsAuthenticated(true);
      localStorage.setItem('sugartown_admin_auth', 'true');
      setPinError(null);
    } else {
      setPinError('Invalid access pin. Please use "sugartown" or "admin".');
    }
  };
  ...
  if (!isAuthenticated) {
    return (
      <div id="admin-login-screen" className="max-w-md mx-auto my-16 sm:my-24 px-4">
        <div className="bento-card p-8 space-y-6 text-center">
          <div className="w-14 h-14 rounded-2xl bg-[#FF5C00] text-white border-2 border-black flex items-center justify-center mx-auto shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]">
          ...
          <div className="space-y-1.5">
            ...
            <h2 className="text-2xl font-black uppercase text-black tracking-tight pt-2">
              Sugartown Management Portal
            </h2>
            <p className="text-xs text-neutral-700 font-medium">
              Enter authorized PIN or master password to access leads & configurations.
            </p>
          </div>

          {pinError && (
            <div className="p-3 rounded-xl bg-rose-50 border-2 border-rose-500 text-rose-800 text-xs font-bold">
              {pinError}
            </div>
          )}
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            <input
              id="admin-pin-input"
              type="password"
              placeholder="Enter PIN (e.g. sugartown or admin)"
              value={pinCode}
              onChange={(e) => setPinCode(e.target.value)}
              className="w-full px-4 py-3 rounded-2xl border-2 border-black text-center font-mono text-sm font-bold tracking-wider shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] focus:outline-none focus:ring-2 focus:ring-[#FF5C00]"
            />

            <button
              id="admin-login-btn"
              type="submit"
              className="bento-btn-primary w-full justify-center py-3.5"
            >
              Authenticate & Enter
            </button>
          </form>
          ...
          <p className="text-[11px] text-neutral-600 font-bold">
            Demo evaluation pin: <code className="font-mono bg-neutral-200 px-1.5 py-0.5 rounded border border-black text-black">sugartown</code> or <code className="font-mono bg-neutral-200 px-1.5 py-0.5 rounded border border-black text-black">admin</code>
          </p>
        </div>
      </div>
    );
